...
void PXLog(NSString *format, ...) {
    ...
    @try {
        ...
        NSString *logMessage = [NSString stringWithFormat:@"[TLinkIOS %@] %@", timestamp, message];
        ...
        for (NSString *path in possiblePaths) {
            if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
                logFilePath = [path stringByAppendingPathComponent:@"TLinkIOS.log"];
                break;
            }
        }
        ...
        if (!logFilePath) {
            logFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"TLinkIOS.log"];
            
            // Attempt to create a logs directory in a location we have access to
            NSString *fallbackPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"TLinkIOSLogs"];
            [[NSFileManager defaultManager] createDirectoryAtPath:fallbackPath 
                                      withIntermediateDirectories:YES 
                                                       attributes:nil 
                                                            error:nil];
            logFilePath = [fallbackPath stringByAppendingPathComponent:@"TLinkIOS.log"];
        }
        ...
        if (@available(iOS 10.0, *)) {
            os_log_t logObject = os_log_create("com.hydra.tlinkios", "general");
