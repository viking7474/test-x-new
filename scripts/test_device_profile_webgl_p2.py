...
def run_source_matrix(matrix: Matrix) -> None:
    ...
    matrix.check("source: DeviceSpec uses canonical identity snapshot", all(token in device_spec_builder for token in ["PXCurrentIdentitySnapshot()", "identitySnapshot.profileID", "identitySnapshot.deviceIDs"]))
    matrix.check("source: DeviceSpec uses canonical snapshot specifications", "specs = identitySnapshot.specs" in device_spec_builder and 'identitySnapshot.source ?: @"device_ids"' in device_spec_builder)
    ...
    matrix.check("source: Canvas uses canonical identity snapshot", "PXCurrentIdentitySnapshot()" in canvas_reader and "snapshot.valid ? snapshot.deviceIDs : nil" in canvas_reader)
