...
static inline void PXFileDebugWebKitTrace(NSString *component) {
    ...
    @autoreleasepool {
        ...
        NSString *path = @"/var/mobile/Library/TLinkIOS/webkit_trace.log";
...
static inline void PXFileDebugLoadMarker(const char *component) {
    ...
    PXFileDebugWritePath("/tmp/tlinkios_loads.log", line, (size_t)len);
}
...
static inline void PXFileDebugAIDA64Log(const char *fmt, ...) {
    ...
    mkdir("/var/mobile/Library/TLinkIOS", 0755);
    int fd = open("/var/mobile/Library/TLinkIOS/aida64_debug.log", O_WRONLY | O_CREAT | O_APPEND, 0644);
