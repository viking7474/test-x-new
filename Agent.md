# Tài liệu Agent - Dự án TLinkIOS (Kiến trúc Chi tiết & Toàn diện)

Tài liệu này cung cấp cái nhìn chi tiết về kiến trúc mã nguồn, phân loại chức năng và vị trí chính xác của các thành phần trong dự án TLinkIOS (WeaponX).
...
- **Hạ tầng Hooking cốt lõi (Kiến trúc quan trọng):**
  - `TLinkIOSTweak/MethodSwizzler.m` (và `.h`)
  - `TLinkIOSTweak/PXScope.m` (và `.h`)
  - `TLinkIOSTweak/PXNativeHookCoordinator.h` (điều phối sở hữu native hook — thay thế HookOwnership.h/gOwner* đã loại bỏ)
  - `TLinkIOSTweak/MobileGestalt.h`
- **Logic Hooking thực tế (Các module đã hoàn thiện):**
  - Khởi tạo chính: `TLinkIOSTweak/Tweak.x`
  - Phiên bản iOS: `TLinkIOSTweak/IOSVersionHooks.x`
  - Phiên bản App: `TLinkIOSTweak/AppVersionHooks.x`
  - Bypass phát hiện Jailbreak: `TLinkIOSTweak/JailbreakBypassHooks.x`
  - Dữ liệu người dùng: `TLinkIOSTweak/UserDefaultsHooks.x`
  - Pin/Năng lượng: `TLinkIOSTweak/BatteryHooks.x`
  - Cấu hình thiết bị: `TLinkIOSTweak/DeviceSpecHooks.x`
  - Mạng/WiFi: `TLinkIOSTweak/WiFiHook.x`, `TLinkIOSTweak/NetworkConnectionTypeHooks.x`
  - Dấu vân tay Canvas: `TLinkIOSTweak/CanvasFingerprintHooks.x`
  - Thời gian khởi động: `TLinkIOSTweak/BootTimeHooks.x`
  - Ngôn ngữ & Múi giờ: `TLinkIOSTweak/LocaleTimeZoneHooks.x`
  - Model thiết bị: `TLinkIOSTweak/DeviceModelHooks.x`
  - Chặn tên miền: `TLinkIOSTweak/DomainBlockingHooks.x`
  - Bảng nhớ tạm (Pasteboard): `TLinkIOSTweak/PasteboardHooks.x`
  - Hệ thống lưu trữ: `TLinkIOSTweak/StorageHooks.x`
  - Chủ đề (Theme): `TLinkIOSTweak/ThemeHooks.x`
  - Các loại UUID: `TLinkIOSTweak/UUIDHooks.x`
  - Giả lập bổ sung: `TLinkIOSTweak/MissingSpoofHooks.x`
  - Hook khởi chạy SpringBoard: `TLinkIOSTweak/SpringBoardLaunchHook.x`
  - Vô hiệu hóa Firebase Perf: `TLinkIOSTweak/FirebasePerfDisableScoped.x`
  - Bảo vệ lớp ObjC: `TLinkIOSTweak/ObjcClassPairGuard.x`
- **File rỗng / Placeholder (Chưa triển khai logic):**
  - `TLinkIOSTweak/KeychainHooks.x` ⚠️
  - `TLinkIOSTweak/AppContainerHooks.x` ⚠️
  - `TLinkIOSTweak/AppGroupHooks.x` ⚠️
  - `TLinkIOSTweak/AppInstallHooks.x` ⚠️
  - `TLinkIOSTweak/CoreDataHooks.x` ⚠️
  - `TLinkIOSTweak/VPNDetectionBypass.x` ⚠️

## 3. Giả lập Vị trí và Cảm biến (Location & Sensor Spoofing)
- **Quản lý GPS:** `common/LocationSpoofingManager.m`
- **Hook Cảm biến (Phân tán):** `TLinkIOSTweak/BatteryHooks.x`, `TLinkIOSTweak/DeviceSpecHooks.x` (Thay đổi thông tin pin, phần cứng).
...
- **Placeholder (Chưa triển khai):** `TLinkIOSTweak/AppContainerHooks.x` ⚠️
